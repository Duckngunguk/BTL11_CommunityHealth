...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final scheduleList = allChildren.where((child) {
      final matchVillage =
          _selectedVillage.isEmpty || child.village == _selectedVillage;
      return matchVillage;
    }).toList();
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          slivers: [
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(result != null
                                  ? 'Đã tải "$result". Kiểm tra thư mục Downloads.'
                                  : 'Không thể xuất CSV.'),
                              backgroundColor:
                                  result != null ? primaryDark : accentRed,
                            ),
                          );
                          NotificationService.instance.sendSystemNotification(
                            title: '📄 Xuất báo cáo thành công',
                            body:
                                'Báo cáo tiêm chủng đã được xuất bởi $reporterName.',
                          );
                        },
                      ),
                    ),
                      ...
                      child: _ExportButton(
                        ...
                        onTap: () async {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(result != null
                                  ? 'Đã mở "$result". Chọn Save để lưu file.'
                                  : 'Đã hủy xuất PDF.'),
                            ),
                          );
                        },
            ...
            SliverToBoxAdapter(
              child: SectionLabel(
                  'DANH SÁCH CẦN TIÊM CHÚNG XÃ ${commune.toUpperCase()}'),
            ),
            ...
            scheduleList.isEmpty
                ? const SliverToBoxAdapter(
                    child: Padding(
                      ...
                      child: Center(
                        child: Text('Không có trẻ nào trong địa bàn này.',
                ...
                children: [
                  Text(label,
                      style: TextStyle(
                          ...
                          color: color)),
                  Text(subtitle,
                      style: const TextStyle(fontSize: 11, color: gray500)),
                ],
              ),
            ),
